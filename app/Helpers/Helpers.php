...
//     {
//         return in_array($permission, session('user_permissions', []));
//     }
// }
...
//check permissions relevant to the title in sidebar
if (!function_exists('userHasPermission')) {
    function userHasPermission($permissionTitle)
    {
        // Get logged-in user's permissions from session
        $userPermissions = session('user_permissions_title', []);

        // Check if the permission exists in the user's permissions
        return in_array($permissionTitle, $userPermissions);
    }
}

//check permission relevant to the module in sidebar
if (!function_exists('userHasModuleAccess')) {
    function userHasModuleAccess($moduleId)
    {
        // Get logged-in user's permissions from session
        $userPermissions = session('user_permissions_title', []);

        // Fetch all permissions of the module from the DB
        ...
        // Check if the user has any permission in this module
