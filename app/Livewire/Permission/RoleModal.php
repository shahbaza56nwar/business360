...
class RoleModal extends Component
{
    ...
    public function submit()
    {
        ...
        // Check if the currently logged-in user has this role
        if (auth()->user()->hasRole($this->role->name)) {
            // Logout user and invalidate session if they are editing their own role
            auth()->logout();
            request()->session()->invalidate();
            request()->session()->regenerateToken();

            // Emit success event before redirecting
